...
function chunk(arr_len, chunksize::Integer)
    return [
        (i, min(i + chunksize - 1, arr_len)) for
        i in range(1, arr_len, step = chunksize)
    ]
end
...
"""
function get_basic_halo_properties(
    halo_pos::Array{T,2},
    model::AbstractForegroundModel,
...
function get_angles(halo_pos::Array{T,2}) where {T}
    ...
    Threads.@threads for i = 1:N_halos
        θ[i], ϕ[i] =
            Healpix.vec2ang(halo_pos[1, i], halo_pos[2, i], halo_pos[3, i])
    end
...
function catalog2map!(m::Map{T,RingOrder}, flux, theta, phi) where {T}
    ...
    Threads.@threads for i_perm = 1:N_halo
        i_halo = perm[i_perm]
...
function BlackBody(
    ν::Unitful.Frequency,
    Tbb::Unitful.Temperature;
    unit_out::Unitful.FreeUnits = u"Jy/sr",
)
...
function fixsen_model(
    x::Unitful.Frequency;
    Tbb::Unitful.Temperature = 18.5u"K",
    unit_out::Unitful.FreeUnits = u"MJy/sr",
)
    B = BlackBody(x, Tbb)
    z0 = 100 / u"cm"
    x2 = uconvert(u"cm*GHz", c_0) / x
    z = uconvert(unit(z0), 1.0 / x2)
...
function write_to_hdf5(filename, fluxes_cen, fluxes_sat)
    hdata = h5open(filename, "w")

    write(hdata, "fluxes_cen", fluxes_cen)
...
function read_fluxes_hdf5(filename)
    hdata = h5open(filename, "r")

    fcen = hdata["fluxes_cen"][:]
    fsat = hdata["fluxes_cen"][:]
    close(hdata)
    return fcen, fsat
end
...
function read_from_hdf5(filename, datalist)
    hdata = h5open(filename, "r")

    datadic = Dict(string => hdata[string][:] for string in datalist)
    close(hdata)
    return datadic
end
